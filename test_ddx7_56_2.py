...
class DSwithIndex(torch.utils.data.Dataset):
...
def compute_losses(
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(iter(test_loader)):
            ...
            if "idx" in results_dict.keys():
                idx_previous: np.ndarray = results_dict["idx"]
                idx_full = np.concatenate((idx, idx_previous), axis=0)
            else:
                idx_full = idx
            results_dict["idx"] = idx_full
...
for ds_name in dataset_list:
    ...
    ds_with_index = DSwithIndex(test_dataset)
    ds_with_index = torch.utils.data.ConcatDataset(
        (ds_with_index for i in range(100))
    )
    test_loader = torch.utils.data.DataLoader(ds_with_index, batch_size=8, num_workers=9)
